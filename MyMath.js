...
class Matrix{
    ...
    sortMatrix(matrix, rowWithYouWillPivot) {
    ...
    GaussJordan(pMatrix, pConstantTerms){
...
function GetModularInverse(number,mod){
    for (let i = 1; i < mod; i++) {
        if((number * i) % mod == 1){
            return i;
        }
